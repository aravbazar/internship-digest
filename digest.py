...
TIMEZONE = ZoneInfo("America/Chicago")  # display only; follows DST itself
...
def main() -> int:
    ...
    # keeps exactly one predictable heartbeat per day: if it fails to arrive,
    # the job is broken rather than the day being quiet.
    if not count and os.environ.get("QUIET_IF_EMPTY") == "true":
        save_seen(seen | current_keys)
        print("0 new roles. Quiet run - no email sent.")
        return 0
